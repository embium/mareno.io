import { c as config, A as AuthError, e as errorResponseSchema, N as NetworkError } from './auth-yELq7bSg.js';
import { a4 as bind_props, W as spread_props, a5 as derived, a6 as props_id, _ as attributes, a7 as lifecycle_function_unavailable, $ as clsx } from './index2-D1ESINYy.js';
import { c as cn, B as Button } from './button-CTC_fQZG.js';
import { b as getAllContexts } from './context-CU6onIa8.js';
import { n as noop, C as Context, w as watch, i as isBrowser$1, S as SPACE, E as ENTER, a as ESCAPE, b as isHTMLElement$1, c as isIOS, d as isElementOrSVGElement } from './noop-BWgdq4c4.js';
import { b as boxWith, c as createBitsAttrs, a as createId, m as mergeProps, d as attachRef, s as simpleBox, g as getDataOpenClosed, e as getDataTransitionAttrs, f as boolToEmptyStrOrUndef, h as executeCallbacks, i as composeHandlers, j as isObject } from './label-CBLPSDC4.js';
import { o as on } from './events-DJ_SkMbQ.js';
...
async function apiRequest(endpoint, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const error = parsedError.success ? parsedError.data : { detail: errorData.detail || "An error occurred" };
      throw new AuthError(error.detail, error.error_code, response.status);
    }
    return await response.json();
  } catch (err) {
    if (err instanceof TypeError || err && typeof err === "object" && "name" in err && err.name === "AbortError") {
      throw new NetworkError(
        err && typeof err === "object" && "name" in err && err.name === "AbortError" ? "Request timed out. Please try again." : "Network error. Please check your connection.",
        err
      );
    }
...
const api = {
  ...
  delete: (endpoint, data, options) => apiRequest(endpoint, {
    ...options,
    method: "DELETE",
    body: data ? JSON.stringify(data) : void 0
  })
            ...
            Dialog_content$1($$renderer4, spread_props([
              {
                ...
                class: cn("fixed top-1/2 left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-popover p-4 text-sm text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 sm:max-w-sm", className)
              },
              restProps,
              {
                get ref() {
...
function Dialog_header($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div${attributes({
      ...
      class: clsx(cn("flex flex-col gap-2", className)),
      ...restProps
    })}>`);
    children?.($$renderer2);
    $$renderer2.push(`<!----></div>`);
      ...
      Dialog_overlay$1($$renderer3, spread_props([
        {
          ...
          class: cn("fixed inset-0 isolate z-50 bg-black/10 duration-100 data-closed:animate-out data-closed:fade-out-0 data-open:animate-in data-open:fade-in-0 supports-backdrop-filter:backdrop-blur-xs", className)
        },
        restProps,
        {
          get ref() {
...
export { Dialog as D, X, Dialog_content as a, api as b, Dialog_header as c, Dialog_title as d, Dialog_description as e, Dialog_footer as f };
//# sourceMappingURL=dialog-CU2liqWk.js.map
