...
<DropdownMenu.Label>
	<span class="flex w-full items-center justify-between">
		<span>Theme</span>
		<span class="flex items-center gap-1 rounded-full border border-[#262626] p-1">
			<button
				onclick={(e) => {
					e.stopPropagation();
					handleThemeClick('light');
				}}
				class="rounded-full p-1.5 transition-colors {mode.current === 'light'
					? 'bg-gray-900 dark:bg-[#fafafa]'
			...
			<button
				...
				class="rounded-full p-1.5 transition-colors {mode.current === 'dark'
					? 'bg-gray-900 dark:bg-[#fafafa]'
					: 'hover:bg-gray-100 dark:hover:bg-[#262626]'}"
