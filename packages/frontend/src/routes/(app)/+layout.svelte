<script lang="ts">
	...
	$effect(() => {
		if (isPremiumRoute && !isUsageValidating) {
...
{:else if authState.isAuthenticated && authState.user}
	<div class="flex h-screen flex-row overflow-hidden bg-background">
		<!-- Sidebar — always visible on desktop, off-canvas on mobile -->
		<DashboardSidebar />

		<!-- Content area — offset top on mobile for the fixed header bar -->
		<div class="flex min-w-0 flex-1 flex-col overflow-hidden pt-14 md:pt-0">
